...
local FunGroupBox = Tabs.Main:AddLeftGroupbox("娱乐", "zap") do
...
local GreenLightRedLightGroup = Tabs.Main:AddLeftGroupbox("一二三木头人", "traffic-light") do
    ...
    GreenLightRedLightGroup:AddButton("完成一二三木头人游戏", function()
    ...
    GreenLightRedLightGroup:AddButton("移除受伤的玩家", function()
    ...
    GreenLightRedLightGroup:AddButton("随机带上受伤的玩家", function()
...
local DalgonaGameGroup = Tabs.Main:AddLeftGroupbox("扣糖饼", "circle") do
...
local MingleGroup = Tabs.Main:AddLeftGroupbox("交融", "users") do
...
local GlassBridgeGroup = Tabs.Main:AddLeftGroupbox("玻璃桥", "bridge") do
    GlassBridgeGroup:AddButton("完成玻璃桥游戏", function()
    ...
    GlassBridgeGroup:AddButton("透视玻璃桥", function()
        if not workspace:FindFirstChild("GlassBridge") then
            Script.Functions.Alert("Game not running")
            return
        end
...
local InformationGroup = Tabs.Main:AddRightGroupbox("信息", "info") do
    if targetlib == "Wind" then
        InformationGroup:AddButton("汉化作者:晓月lol", Script.Functions.JoinDiscordServer)
    end
...
local HideAndSeekGroup = Tabs.Main:AddRightGroupbox("刀与钥匙", "search") do
    ...
    HideAndSeekGroup:AddButton("传送至安全点", function()
        if Script.GameState ~= "HideAndSeek" then 
            Script.Functions.Alert("Game not running!")
            return
        end
...
local JumpRopeGroup = Tabs.Main:AddRightGroupbox("跳绳") do
    ...
    JumpRopeGroup:AddButton("禁用跌倒检测 [测试]", function()
        local suc = pcall(function()
            workspace.JumpRope.FallColllisionYClient:Destory()
...
local RebelGroup = Tabs.Main:AddRightGroupbox("叛乱", "sword") do
    ...
    RebelGroup:AddButton("带走所有守卫", function()
        pcall(function()
    ...
    RebelGroup:AddToggle("GunMods", { Text = "枪械杀戮光环", Default = false })
...
local AutoShootGroup = Tabs.Main:AddRightGroupbox("自动射击", "target") do
    ...
    AutoShootGroup:AddToggle("AutoShootWallcheck", { Text = "检测墙壁", Default = true })
...
local EmotesGroup = Tabs.Misc:AddRightGroupbox("表情", "smile") do
    ...
    EmotesGroup:AddButton("选择表情", function()
    ...
    EmotesGroup:AddButton("停止表情", function()
        if Script.Temp.EmoteTrack and typeof(Script.Temp.EmoteTrack) == "Instance" and Script.Temp.EmoteTrack:IsA("AnimationTrack") then
            pcall(function() Script.Temp.EmoteTrack:Stop() end)
            Script.Temp.EmoteTrack = nil
        end
...
local PerformanceGroupBox = Tabs.Misc:AddRightGroupbox("游戏表现", "gauge") do
...
local AntiDeathGroup = Tabs.Other:AddRightGroupbox("无敌", "skull") do
